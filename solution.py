...
diagonal_units = [diag_cross(r,c) for r, c in zip((rows, reversed(rows)), (cols, cols))]
...
def search(values):
    ...
    for key, value in values.items():
        if len(value) < min_len and len(value) > 1:
            fewest_box = []
            fewest_box.append(key)
            min_len = len(value)
